...
export default function SignIn({ signedInFunc, canceledFunc }) {
    ...
    const errorMessage = errorIsVisible ? (
        <div className="absolute bottom-[-.4rem] left-[30%] text-red-400 text-lg">Please fill in all required fields</div>
    ) : (
        null
    );
        ...
        existingAccount ? (
            <div className="fixed top-0 left-0 w-screen h-screen bg-black/50 flex justify-center items-center z-20">
                <div className="w-[50%] h-[55%] bg-slate-100 border-4 border-solid border-gray-200 rounded-3xl grid grid-cols-[25%_1fr] grid-rows-[25%_repeat(4,1fr)] gap-y-2">
                    ...
                    <button className='col-span-2 place-self-center bg-gray-800 text-white rounded-3xl w-36 h-11 text-xl hover:opacity-50 ease-in-out duration-200'>Log In</button>
